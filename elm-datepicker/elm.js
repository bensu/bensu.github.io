...
Elm.Date = Elm.Date || {};
Elm.Date.Util = Elm.Date.Util || {};
Elm.Date.Util.make = function (_elm) {
   "use strict";
   _elm.Date = _elm.Date || {};
   _elm.Date.Util = _elm.Date.Util || {};
   if (_elm.Date.Util.values)
   return _elm.Date.Util.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Date.Util",
   ...
   $WebAPI$Date = Elm.WebAPI.Date.make(_elm);
   var extractVals = function (a) {
      return function () {
         var _v0 = A2($Array.get,0,a);
         switch (_v0.ctor)
         {case "Just":
            return function () {
                 var _v2 = A2($Array.get,1,a);
                 switch (_v2.ctor)
                 {case "Just":
                    return function () {
                         var _v4 = A2($Array.get,2,a);
                         switch (_v4.ctor)
                         {case "Just":
   ...
   var maybeInt = function (s) {
      return $Result.toMaybe($String.toInt(s));
   };
   var toMonth = function (n) {
      return $WebAPI$Date.toMonth(n - 1);
   };
   var equalDates = F2(function (d1,
   d2) {
      return _U.eq($Date.day(d1),
      $Date.day(d2)) && (_U.eq($Date.month(d1),
      $Date.month(d2)) && _U.eq($Date.year(d1),
      $Date.year(d2)));
   });
   var addToGroups = F2(function (groups,
   date) {
      return function () {
         var _v6 = $Date.dayOfWeek(date);
         switch (_v6.ctor)
         {case "Sun":
            return A2($List._op["::"],
              _L.fromArray([date]),
         ...
         return function () {
            ...
            {case "Just":
               ...
               case "Nothing":
               return _L.fromArray([_L.fromArray([date])]);}
            _U.badCase($moduleName,
            "between lines 66 and 71");
         }();
      }();
   });
   var groupToWeeks = F2(function (daysLeft,
   groups) {
      return function () {
         var _v11 = $List.head(daysLeft);
         switch (_v11.ctor)
         {case "Just":
            return function () {
                 var groups$ = A2(addToGroups,
                 groups,
                 _v11._0);
                 return function () {
                    var _v13 = $List.tail(daysLeft);
                    switch (_v13.ctor)
                    {case "Just":
                       return A2(groupToWeeks,
                         _v13._0,
                         groups$);
                       case "Nothing": return groups$;}
                    _U.badCase($moduleName,
   ...
   day) {
      return function () {
         var parts = {_: {}
                     ...
                     ,millisecond: 0
                     ,minute: 1
                     ,month: $WebAPI$Date.fromMonth(month)
                     ,second: 0
                     ,year: year};
         return A2($WebAPI$Date.fromParts,
         $WebAPI$Date.Local,
         parts);
      }();
   });
   var addMonths = F2(function (_v15,
   a) {
      return function () {
         switch (_v15.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 _v15._0,
                 1);
                 var date$ = A3($WebAPI$Date.offsetMonth,
                 $WebAPI$Date.Local,
                 a,
                 date);
                 return {ctor: "_Tuple2"
   ...
   var parseDate = function (s) {
      return function () {
         ...
         "/",
         s);
         return _U.eq($List.length(parts),
         3) ? function () {
            var parts$ = A2($List.filterMap,
            maybeInt,
            parts);
            return _U.eq($List.length(parts$),
            3) ? function () {
               var _v19 = extractVals($Array.fromList(parts$));
               switch (_v19.ctor)
               {case "Just":
                  switch (_v19._0.ctor)
                    {case "_Tuple3":
                       return $Maybe.Just(A3(newDate,
                         _v19._0._0,
                         toMonth(_v19._0._1),
                         _v19._0._2));}
                    break;
   ...
   var addDays = F2(function (date,
   n) {
      return A2($WebAPI$Date.offsetTime,
      $Basics.toFloat((n * 24 + 3) * 3600000),
      date);
   });
   var allDaysInMonth = F2(function (month,
   year) {
      return function () {
         var firstDate = A3(newDate,
         year,
         month,
         1);
   ...
   var allWeeksInMonth = F2(function (month,
   year) {
      return $List.reverse(A2(groupToWeeks,
      A2(allDaysInMonth,month,year),
      _L.fromArray([_L.fromArray([])])));
   });
   var monthData = function (m) {
      return function () {
         switch (m.ctor)
         {case "Apr":
            return {ctor: "_Tuple2"
            ...
            return {ctor: "_Tuple2"
                   ...
                   ,_1: "September"};}
         _U.badCase($moduleName,
         "between lines 13 and 26");
      }();
   };
   var monthNumber = function (m) {
      return $Basics.fst(monthData(m));
   };
   var monthName = function (m) {
      return $Basics.snd(monthData(m));
   };
   var monthNamesShort = _L.fromArray(["Jan"
                                      ,"Feb"
                                      ,"Mar"
                                      ,"Apr"
                                      ,"May"
...
Elm.Main.make = function (_elm) {
   ...
   $Widget$Datepicker = Elm.Widget.Datepicker.make(_elm);
   var main = $StartApp$Simple.start({_: {}
                                     ,model: $Widget$Datepicker.initValue
                                     ,update: $Widget$Datepicker.update
                                     ,view: $Widget$Datepicker.view});
...
Elm.WebAPI.Date = Elm.WebAPI.Date || {};
Elm.WebAPI.Date.make = function (_elm) {
   "use strict";
   _elm.WebAPI = _elm.WebAPI || {};
   _elm.WebAPI.Date = _elm.WebAPI.Date || {};
   if (_elm.WebAPI.Date.values)
   return _elm.WebAPI.Date.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "WebAPI.Date",
   $Basics = Elm.Basics.make(_elm),
   $Date = Elm.Date.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$WebAPI$Date = Elm.Native.WebAPI.Date.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Time = Elm.Time.make(_elm);
   var utcString = $Native$WebAPI$Date.utcString;
   var isoString = $Native$WebAPI$Date.isoString;
   var timeString = $Native$WebAPI$Date.timeString;
   var dateString = $Native$WebAPI$Date.dateString;
   var day = 24 * $Time.hour;
   var inDays = function (days) {
      return days / day;
   };
   var week = 7 * day;
   var inWeeks = function (weeks) {
      return weeks / week;
   };
   var offsetMonthUTC = $Native$WebAPI$Date.offsetMonthUTC;
   var offsetMonthLocal = $Native$WebAPI$Date.offsetMonthLocal;
   var offsetMonth = function (zone) {
      return function () {
         switch (zone.ctor)
         {case "Local":
            return offsetMonthLocal;
            case "UTC":
            return offsetMonthUTC;}
         _U.badCase($moduleName,
         "between lines 364 and 366");
      }();
   };
   var offsetYearUTC = $Native$WebAPI$Date.offsetYearUTC;
   var offsetYearLocal = $Native$WebAPI$Date.offsetYearLocal;
   var offsetYear = function (zone) {
      return function () {
         switch (zone.ctor)
         {case "Local":
            return offsetYearLocal;
            case "UTC":
            return offsetYearUTC;}
         _U.badCase($moduleName,
         "between lines 321 and 323");
      }();
   };
   var offsetTime = F2(function (time,
   date) {
      return $Date.fromTime($Date.toTime(date) + time);
   });
   var fromDay = function (day) {
      return function () {
         switch (day.ctor)
         {case "Fri": return 5;
            case "Mon": return 1;
            ...
            case "Wed": return 3;}
         _U.badCase($moduleName,
         "between lines 266 and 278");
      }();
   };
   var toDay = function ($int) {
      return function () {
         var clamped = A2($Basics.rem,
         $int,
         7);
         var positive = _U.cmp(clamped,
         0) < 0 ? clamped + 7 : clamped;
         return function () {
            switch (positive)
            ...
            return $Debug.crash("This is unreachable.");
         }();
      }();
   };
   var fromMonth = function (month) {
      return function () {
         switch (month.ctor)
         {case "Apr": return 3;
            case "Aug": return 7;
            ...
            case "Sep": return 8;}
         _U.badCase($moduleName,
         "between lines 224 and 236");
      }();
   };
   var toMonth = function ($int) {
      return function () {
         var clamped = A2($Basics.rem,
         $int,
         12);
         var positive = _U.cmp(clamped,
         0) < 0 ? clamped + 12 : clamped;
         return function () {
            switch (positive)
   ...
   var dayOfWeekUTC = $Native$WebAPI$Date.dayOfWeekUTC;
   var dayOfWeek = F2(function (timezone,
   date) {
      return function () {
         switch (timezone.ctor)
         {case "Local":
            return $Date.dayOfWeek(date);
            case "UTC":
            return toDay(dayOfWeekUTC(date));}
         _U.badCase($moduleName,
         "between lines 180 and 185");
      }();
   });
   var toPartsUtc = $Native$WebAPI$Date.toPartsUtc;
   var toPartsLocal = $Native$WebAPI$Date.toPartsLocal;
   var toParts = function (zone) {
      return function () {
         switch (zone.ctor)
         {case "Local":
            return toPartsLocal;
            case "UTC": return toPartsUtc;}
         _U.badCase($moduleName,
         "between lines 160 and 162");
      }();
   };
   var fromPartsUtc = $Native$WebAPI$Date.fromPartsUtc;
   var fromPartsLocal = $Native$WebAPI$Date.fromPartsLocal;
   var fromParts = function (zone) {
      return function () {
         switch (zone.ctor)
         {case "Local":
            return fromPartsLocal;
            case "UTC":
            return fromPartsUtc;}
         _U.badCase($moduleName,
         "between lines 139 and 141");
   ...
   var timezoneOffsetInMinutes = $Native$WebAPI$Date.timezoneOffsetInMinutes;
   var timezoneOffset = function (date) {
      return timezoneOffsetInMinutes(date) * $Time.minute;
   };
   var UTC = {ctor: "UTC"};
   var Local = {ctor: "Local"};
   var now = $Native$WebAPI$Date.now;
   var current = $Native$WebAPI$Date.current;
   _elm.WebAPI.Date.values = {_op: _op
                             ,current: current
                             ,now: now
...
Elm.Widget.Datepicker = Elm.Widget.Datepicker || {};
Elm.Widget.Datepicker.make = function (_elm) {
   "use strict";
   _elm.Widget = _elm.Widget || {};
   _elm.Widget.Datepicker = _elm.Widget.Datepicker || {};
   if (_elm.Widget.Datepicker.values)
   return _elm.Widget.Datepicker.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Widget.Datepicker",
   $Basics = Elm.Basics.make(_elm),
   $Date = Elm.Date.make(_elm),
   $Date$Util = Elm.Date.Util.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "Blur":
            return _U.replace([["isOn"
                               ,false]],
              model);
            case "DeltaMonth":
            return function () {
                 var _v4 = model.selectMonth;
                 switch (_v4.ctor)
                 {case "Just":
                    switch (_v4._0.ctor)
                      {case "_Tuple2":
                         return function () {
                              ...
                              return _U.replace([["selectMonth"
                                                 ,$Maybe.Just(A2($Date$Util.addMonths,
                                                 {ctor: "_Tuple2"
                                                 ,_0: _v4._0._0
                                                 ,_1: _v4._0._1},
                                                 delta))]],
                 ...
                 _U.badCase($moduleName,
                 "between lines 230 and 237");
              }();
            case "Focus":
            return _U.replace([["isOn",true]
                              ,["selectMonth"
                               ,function () {
                                  var _v9 = model.selectMonth;
                                  switch (_v9.ctor)
                                  {case "Just":
            ...
            case "Select":
            return _U.replace([["date"
                               ,$Maybe.Just(action._0)]
                              ,["isOn",false]
                              ,["selectMonth"
                               ,$Maybe.Just({ctor: "_Tuple2"
                                            ,_0: $Date.month(action._0)
                                            ,_1: $Date.year(action._0)})]],
              model);}
         _U.badCase($moduleName,
         "between lines 215 and 237");
      }();
   });
   var renderDate = function (date) {
      return function () {
         ...
         {case "Just":
            return function () {
                 var strs = A2($List.map,
                 function (f) {
                    return $Basics.toString(f(date._0));
                 },
                 _L.fromArray([$Date.day
                              ,function (d) {
                                 return 1 + $Date$Util.monthNumber($Date.month(d));
                              }
                              ,$Date.year]));
   ...
   var completeWeek = function (week) {
      return _U.eq($List.length(week),
      7) ? week : function () {
         var _v13 = $List.head(week);
         switch (_v13.ctor)
         {case "Just":
            return function () {
                 switch (_v13._0.ctor)
                 {case "Just":
                    return function () {
                         var _v17 = $Date.dayOfWeek(_v13._0._0);
                         switch (_v17.ctor)
                         {case "Sun": return week;}
                         return completeWeek(A2($List._op["::"],
                         $Maybe.Nothing,
                         week));
                      }();
                    case "Nothing":
   ...
   var isSelected = F2(function (selected,
   date) {
      return function () {
         switch (selected.ctor)
         {case "Just":
            return A2($Date$Util.equalDates,
              selected._0,
              date);
            case "Nothing": return false;}
         _U.badCase($moduleName,
   ...
   var dayLabel = function (day) {
      return A2($Html.th,
      _L.fromArray([$Html$Attributes.$class(day.isWeekend ? "ui-datepicker-week-end" : "")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([]),
      _L.fromArray([$Html.text(day.name)]))]));
   };
   var days = A3($List.map2,
   F2(function (n,b) {
      return {_: {}
             ,isWeekend: b
             ,name: n};
   ...
   var title = function (name) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("ui-datepicker-title")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("ui-datepicker-month")]),
      _L.fromArray([$Html.text(name)]))]));
   };
   var Icon = F3(function (a,b,c) {
      return {_: {}
             ,iconClass: b
             ,iconName: a
             ,linkName: c};
   });
   var DeltaMonth = function (a) {
      return {ctor: "DeltaMonth"
             ,_0: a};
   };
   var icon = F2(function (address,
   move) {
      return function () {
         var i = function () {
            switch (move.ctor)
         ...
         var iconClassList = $Html$Attributes.classList(_L.fromArray([{ctor: "_Tuple2"
                                                                      ,_0: "ui-corner-all"
                                                                      ,_1: true}
                                                                     ,{ctor: "_Tuple2"
                                                                      ,_0: "ui-icon"
         ...
         _L.fromArray([A2($Html.span,
         _L.fromArray([iconClassList]),
         _L.fromArray([$Html.text(i.iconName)]))]));
      }();
   });
   ...
   var dayNumber = F3(function (address,
   selected,
   date) {
      return function () {
         switch (date.ctor)
         {case "Just":
            return A2($Html.td,
              ...
              _L.fromArray([$Html$Attributes.classList(_L.fromArray([{ctor: "_Tuple2"
                           ...
                           address,
                           Select(date._0))]),
              _L.fromArray([$Html.text($Basics.toString($Date.day(date._0)))]))]));
            case "Nothing":
            return A2($Html.td,
              _L.fromArray([]),
              _L.fromArray([]));}
   ...
   var weekRow = F3(function (address,
   selected,
   week) {
      return A2($Html.tr,
      _L.fromArray([]),
      A2($List.map,
      A2(dayNumber,address,selected),
      completeWeek(A2($List.map,
      function (d) {
         return $Maybe.Just(d);
      },
      week))));
   });
   var calendar = F3(function (address,
   selected,
   _v23) {
      return function () {
         switch (_v23.ctor)
         {case "_Tuple2":
            return A2($Html.table,
              _L.fromArray([$Html$Attributes.$class("ui-datepicker-calendar")]),
              _L.fromArray([A2($Html.thead,
                           _L.fromArray([]),
                           _L.fromArray([A2($Html.tr,
                           _L.fromArray([]),
                           A2($List.map,dayLabel,days))]))
                           ,A2($Html.tbody,
                           _L.fromArray([]),
                           A2($List.map,
                           A2(weekRow,address,selected),
                           A2($Date$Util.allWeeksInMonth,
                           _v23._0,
                           _v23._1)))]));}
         _U.badCase($moduleName,
         "between lines 150 and 156");
      }();
   });
   ...
   var Next = {ctor: "Next"};
   var Prev = {ctor: "Prev"};
   var header = F2(function (address,
   _v27) {
      return function () {
         switch (_v27.ctor)
         {case "_Tuple2":
            return A2($Html.div,
              _L.fromArray([$Html$Attributes.$class("ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all")]),
              _L.fromArray([A2(icon,
                           address,
                           Prev)
                           ,A2(icon,address,Next)
                           ,title(A2($Basics._op["++"],
                           $Date$Util.monthName(_v27._0),
                           A2($Basics._op["++"],
                           " - ",
                           $Basics.toString(_v27._1))))]));}
         _U.badCase($moduleName,
   ...
   datepicker) {
      ...
      _L.fromArray([A2($Html.input,
                   _L.fromArray([$Html$Attributes.$class("ui-datepicker-input")
                                ,A3($Html$Events.on,
                                "change",
                                A2($Json$Decode.at,
                                _L.fromArray(["target"
                                ...
                                function (v) {
                                   ...
                                   address,
                                   function () {
                                      var _v31 = $Date$Util.parseDate(v);
                                      switch (_v31.ctor)
                                      {case "Just":
                                         return Select(_v31._0);
                                ...
                                ,$Html$Attributes.value(renderDate(datepicker.date))]),
                   _L.fromArray([]))
                   ,function () {
                      var _v33 = datepicker.selectMonth;
                      switch (_v33.ctor)
                      {case "Just":
                         return A2($Html.div,
                           _L.fromArray([$Html$Attributes.$class("ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all")
                                        ,A2($Html$Attributes.attribute,
                                        "aria-live",
                                        "polite")
                                        ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                              ,_0: "display"
                                                                              ,_1: datepicker.isOn ? "block" : "none"}
                                                                             ,{ctor: "_Tuple2"
                                                                              ,_0: "position"
                                                                              ,_1: "absolute"}
                                                                             ,{ctor: "_Tuple2"
                         ...
                         return A2($Html.div,
                           ...
                           _L.fromArray([]));}
                      _U.badCase($moduleName,
                      "between lines 195 and 211");
                   }()]));
   });
   var initValue = {_: {}
                   ,date: $Maybe.Nothing
                   ,isOn: false
                   ,selectMonth: $Maybe.Nothing};
   var Datepicker = F3(function (a,
   b,
   c) {
      return {_: {}
             ,date: a
   ...
   _elm.Widget.Datepicker.values = {_op: _op
                                   ,view: view
                                   ,update: update
                                   ,initValue: initValue
