...
// MATH CONSTANTS AND FUNCTIONS
   ...
   function add(a, b) { return [ a[0] + b[0], a[1] + b[1], def(a[2]) + def(b[2]) ]; }
   ...
   function mult(x,y) {
	 ...
	 for (prop in y)
      ...
      function dot(a,b) {
         var result = 0, i = min(a.length, b.length);
	 while (i--)
	    result += a[i] * b[i];
         return result;
      }
      ...
      if (! xIsArray && ! yIsArray) {
         ...
         dst = max(x.length, y.length)  < 16 ? dot(x, y)
   ...
   var noise = (function() {
      ...
      return function(x, y, z) {
         ...
         if (Array.isArray(x))
            return improvedNoise.noise(x[0], def(x[1]), def(x[2]));
         return noise2(x, 0);
      }
   })();
   ...
   function sCurve(t) { return t < 0 ? 0 : t > 1 ? 1 : t * t * (3 - t - t); }
   function saw(t) { t = 2*t % 2; return t<1 ? t : 2-t; }
   function sbias(t, p) { return bias(.5 + .5 * t, p) * 2 - 1; }
   function scale(a, s) { return isDef(a[2]) ? [ s * a[0], s * a[1], s * a[2] ] : [ s * a[0], s * a[1] ]; }
   ...
   function subtract(a, b) { return isDef(a[2]) ? [ a[0]-b[0], a[1]-b[1], a[2]-b[2] ] : [ a[0]-b[0], a[1]-b[1] ]; }
   ...
   function Rotation(m) {
      ...
      let d = Math.pow(-determinant(m), 1/3);
      this.m = [];
      for (let i = 0 ; i < m.length ; i++)
         this.m.push(m[i] / d);

      this.rotateAbout = function(p, q) {
         var x = q[0] - p[0], y = q[1] - p[1], z = q[2] - p[2];
         return [ p[0]  +  x * this.m[0]  +  y * this.m[3]  +  z * this.m[6],
                  p[1]  +  x * this.m[1]  +  y * this.m[4]  +  z * this.m[7],
                  p[2]  +  x * this.m[2]  +  y * this.m[5]  +  z * this.m[8] ];
      }
   }
...
// IMAGE PROCESSING.
   ...
   function mirrorFlip(src, nCols) {
      var nRows = (src.length >> 2) / nCols;
      for (let y = 0 ; y < nRows   ; y++)
      for (let x = 0 ; x < nCols/2 ; x++) {
         let i =           x + nCols * y << 2;
         let j = nCols-1 - x + nCols * y << 2;
         for (let k = 0 ; k < 3 ; k++) {
            let temp = src[i+k];
            src[i+k] = src[j+k];
            src[j+k] = temp;
...
// 2D GEOMETRY UTILITIES.
   ...
   function nudgeCurveInit(curve, pt, i0) {
      var n = curve.length;
      ...
      curve._nudgeCurveData = {i0:i0, im:im};
   }

   function nudgeCurve(curve, pt) {
      var i0 = curve._nudgeCurveData.i0,
          im = curve._nudgeCurveData.im,
          n  = curve.length;
   ...
   */
   ...
   function makeSpline(keys, dst, ptsPerKey) {
      var spline = dst;
      if (spline === undefined)
         spline = [];

      var N = def(ptsPerKey, 10);
      var nk = keys.length;
      var ns = 0;
   ...
   function computeCurveLength(curve, i0) {
      ...
      for (var i = (isDef(i0) ? i0 : 0) ; i < curve.length - 1 ; i++) {
         ...
         var dz = curve[i][2] ? curve[i+1][2] - curve[i][2] : 0;
...
// 3D GEOMETRY UTILITIES.
   ...
   function computeNormals(V, T) {
      let N = newArray(V.length);
      for (let n = 0 ; n < T.length ; n += 3) {
	     ...
	     u = CT.cross(CA, AB), uu = max(CT.dot(u, u), .00001),
	     v = CT.cross(AB, BC), vv = max(CT.dot(v, v), .00001),
	     w = CT.cross(BC, CA), ww = max(CT.dot(w, w), .00001);
	 for (let i = 0 ; i < 3 ; i++) {
	    N[a + i] += u[i] / uu;
	    N[b + i] += v[i] / vv;
	    N[c + i] += w[i] / ww;
      ...
      for (let n = 0 ; n < N.length ; n += 3) {
         let v = CT.normalize([N[n], N[n+1], N[n+2]]);
	 for (let i = 0 ; i < 3 ; i++)
	    N[n + i] = v[i];
      }
      return N;
   ...
   function resampleVertices(V, T, N) {
      let W = newArray(V.length);
      let A = newArray(V.length / 3);
      for (let n = 0 ; n < T.length ; n += 3) {
         let a = 3 * T[n], b = 3 * T[n+1], c = 3 * T[n+2];
	 A[a/3] += 2;
	 A[b/3] += 2;
	 ...
	 for (let i = 0 ; i < 3 ; i++) {
      ...
      for (let n = 0 ; n < W.length ; n += 3)
	    ...
	    W[n + i] /= A[n/3];
      for (let n = 0 ; n < W.length ; n += 3) {
         let nn = [N[n], N[n+1], N[n+2]];
         let dv = [W[n] - V[n], W[n+1] - V[n+1], W[n+2] - V[n+2]];
	 let d = CT.dot(dv, nn);
	 for (let i = 0 ; i < 3 ; i++)
	    W[n + i] -= d * nn[i];
      }
...
// VARIOUS MANIPULATIONS OF HTML ELEMENTS.
   ...
   function smooth(signal, width, nPasses, isLoop) {
   ...
   // Outputs a matrix that can be used to convert from a space in (roughly) pixel units
   // to a space in more generic 3D coordinates
   function pixelSpaceTo3DSpaceMatrix() {
      // Look at this as transverse, since CT matrices are column-major
      let halfWidth = width()/2;
      return [1/halfWidth, 0, 0, 0,
            0, -1/halfWidth, 0, 0,
            0, 0, 1/halfWidth, 0,
            (_g.panX - halfWidth)/halfWidth, (-_g.panY + height() / 2)/halfWidth, 0, 1];
   }
